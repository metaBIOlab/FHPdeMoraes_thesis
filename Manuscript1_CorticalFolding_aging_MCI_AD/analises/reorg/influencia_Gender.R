...
ttest_AreaE <-
  t.test(SmoothArea ~ Gender, data = dados_hemi_v1_v1_CTL, var.equal = FALSE)
Cohend_AreaE <- cohen.d(SmoothArea ~ Gender, data = dados_hemi_v1_v1_CTL)
ttest_AreaE
Cohend_AreaE
...
ttest_logk <-
  t.test(log_kteorico ~ Gender, data = dados_hemi_v1_v1_CTL, var.equal = FALSE)
Cohend_logk <- cohen.d(log_kteorico ~ Gender, data = dados_hemi_v1_v1_CTL)
ttest_logk
Cohend_logk
...
hist_AreaE <-
  ggplot(data = dados_hemi_v1_v1_CTL, aes(SmoothArea, fill = Gender)) +
  geom_histogram(alpha = .6, position = "identity") +
  labs(
...
hist_logk <-
  ggplot(data = dados_hemi_v1_v1_CTL, aes(log_kteorico, fill = Gender)) +
  geom_histogram(alpha = .6, position = "identity") +
  labs(
    title = "log Offset k",
    subtitle = paste(
...
bxplt_AreaE <-
  ggplot(data = dados_hemi_v1_v1_CTL, aes(Gender, SmoothArea, color = Gender)) +
  geom_boxplot() +
  labs(
    title = "Exposed grey matter surface area",
    subtitle = paste(
...
bxplt_logk <-
  ggplot(data = dados_hemi_v1_v1_CTL, aes(Gender, log_kteorico, color = Gender)) +
  geom_boxplot() +
  labs(
    title = "log Offset k",
    subtitle = paste(
...
LM_Gender <-
  lm(formula = log_kteorico ~ Gender * Diagnostic,
...
dados_lm_gender <- dados_hemi_v1  %>%
  group_by(Gender) %>%
  do(fit_gender = lm(log10(sqrt(AvgThickness) * TotalArea) ~ log10(SmoothArea), data = .))
...
dados_lm_gender_age <-
  ...
  do(fit_gender_age = lm(log10(sqrt(AvgThickness) * TotalArea) ~ log10(SmoothArea), data = .))
...
gender_age_intercept <-
  filter(gender_age_Coef, term == "log10(SmoothArea)")
...
dados_lm_Gender_age_t <-
  ...
  do(fit_Gender_age_t = lm(log10(sqrt(AvgThickness) * TotalArea) ~ log10(SmoothArea) + Gender + log10(SmoothArea)*Gender, data = .))
