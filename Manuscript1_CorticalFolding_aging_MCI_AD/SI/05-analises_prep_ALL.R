...
dados_BHRCS$Session <- as.double(dados_BHRCS$Session)
...
dados_datasetscomp <-
  ...
  # full_join(dados_HPC) %>%
  full_join(dados_ACC) %>%
  full_join(dados_caltechucsf) %>%
  full_join(dados_zk) %>%
  full_join(dados_micro) %>%
  full_join(dados_BHRCS) %>%
  full_join(dados_ds000030)
...
dados_datasetscomp <- dados_datasetscomp %>%
  ...
  filter(
    ...
    localGI != 0,!is.infinite(TotalArea_corrected),!is.na(AvgThickness), !is.nan(TotalArea_corrected)
    ) %>%
  droplevels() %>%
  unique()
