...
decay_logTotalArea <- filter(dados_datasetscomp, Diagnostic == "CTL", !is.na(log10(TotalArea_corrected)), !is.nan(log10(TotalArea_corrected)), !is.infinite(log10(TotalArea_corrected))) %>%
  droplevels() %>%
  group_by(ROI, Sample) %>%
  do(fit_decay_logTotalArea = tidy(rlm(TotalArea_corrected ~ Age, data = .),conf.int=TRUE)) %>%
...
decay_logExposedArea <- filter(dados_datasetscomp, Diagnostic == "CTL", !is.na(logExposedArea_corrected), !is.nan(logExposedArea_corrected), !is.infinite(logExposedArea_corrected)) %>%
  droplevels() %>%
  group_by(ROI, Sample) %>%
  do(fit_decay_logExposedArea = tidy(rlm(ExposedArea_corrected ~ Age, data = .), conf.int = TRUE)) %>%
...
dados_datasetscomp <- full_join(dados_datasetscomp, decay_AvgThickness) %>%
  ...
  mutate(
    ...
    TotalArea_age_decay = TotalArea_corrected - c_logExposedArea * (Age - Age.cor),
    ExposedArea_age_decay = ExposedArea_corrected - c_logExposedArea * (Age - Age.cor),
    ...
    k_age_decay = sqrt(AvgThickness_age_decay) * TotalArea_age_decay / (ExposedArea_age_decay ^ 1.25),
    K_age_decay2 = K_corrected - c_K * (Age - Age.cor),
    K_age_decay = 1 / 4 * log10(AvgThickness_age_decay^2)  + log10(TotalArea_age_decay) - 5 / 4 * log10(ExposedArea_age_decay),
