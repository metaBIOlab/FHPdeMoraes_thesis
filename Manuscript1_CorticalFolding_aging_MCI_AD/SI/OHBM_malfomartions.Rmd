...
dados_datasetscomp_ALL <- dados_datasetscomp %>%
  filter(!is.na(AvgThickness),
         ...
         !is.na(Diagnostic),
         Sample == "AOMICPIOP1" | Sample == "AOMICPIOP2" | Sample == "AHEAD" | Sample == "ADNI" | Sample == "HCPr900" | Sample == "NKI" | Sample == "OASIS" | Sample == "IDOR")
...
a <- ggplot(filter(dados_datasetscomp, ROI == "hemisphere", Family == "Hominidae"), aes(x = ExposedArea, y = TotalArea*sqrt(AvgThickness), color = Diagnostic, fill = Diagnostic)) +
  geom_point(aes(color = "white", shape = Sample)) +
  scale_x_log10() +
  scale_y_log10() +
  labs(x = "Exposed cortical surface area (mm²)", y = "sqrt(AvgThickness) * Total cortical surface area") +
  theme_minimal() +
...
ggarrange(ggarrange(figa, figb,labels = c("A","B"), nrow = 1, ncol = 2, font.label = list(size = 11), common.legend = TRUE, legend = "top"))
...
```{r}
amostra_Coef <- lm(log10(TotalArea *sqrt(AvgThickness)) ~ log10(ExposedArea), data = filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL"), na.action = na.omit)
...
cov <- filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL") %>%
  dplyr::select(c(Age, AvgThickness, TotalArea, ExposedArea)) %>%
  cov()

mean <- c(mean(filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL")$Age), mean(filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL")$AvgThickness), mean(filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL")$TotalArea), mean(filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL")$ExposedArea))

simulated_values <-
  as.data.frame(mvrnorm(n = 10000, mu = mean, Sigma = cov)) %>%
  filter(
    Age > min(
...
```{r}
ggplot(filter(dados_datasetscomp_ALL, ROI == "hemisphere", Diagnostic == "CTL"), aes(x=ExposedArea, y =TotalArea*sqrt(AvgThickness))) +
  geom_point(aes(color = "white", alpha = 0.3)) +
...
paste("N dados simulados = ", length(simulated_values))
...
a <- ggplot(filter(dados_datasetscomp_pred, ROI == "hemisphere", Family == "Hominidae"), aes(x = ExposedArea, y = TotalArea*sqrt(AvgThickness), color = Diagnostic, fill = Diagnostic)) +
  geom_point(aes(color = "white", shape = Sample, alpha = ifelse(Diagnostic == "CTL-simulated", 0.1, 0.3))) +
  scale_x_log10() +
  scale_y_log10() +
...
ggarrange(figab, figcd,nrow = 2, ncol = 1)
