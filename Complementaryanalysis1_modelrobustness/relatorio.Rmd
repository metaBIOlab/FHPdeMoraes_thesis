...
## Comring the usage of longitudinal correction
...
lm_fit_comp_idor <- long_comp %>%
  ...
  do(fit_comp_idor = tidy(lm(log10(TotalArea*sqrt(AvgThickness)) ~ log10(ExposedArea), data = ., na.action = na.omit), conf.int=TRUE)) %>% unnest(cols = c(fit_comp_idor))
