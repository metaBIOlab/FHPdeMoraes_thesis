...
### THICKNESS ####
lh_pial_thickness <-
  unique(read_delim(str_c(path,"lh_pial_thickness.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
rh_pial_thickness <-
  unique(read_delim(str_c(path,"rh_pial_thickness.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
### TOTAL AREA ####
lh_pial_surfarea <-
  unique(read_delim(str_c(path,"lh_pial_surfarea.txt"),
                    "\t",
                    escape_double = FALSE,
                    trim_ws = TRUE
  ))
...
rh_pial_surfarea <-
  unique(read_delim(str_c(path,"rh_pial_surfarea.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
lh_pial_volume <-
  unique(read_delim(str_c(path,"lh_pial_volume.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
rh_pial_volume <-
  unique(read_delim(str_c(path,"rh_pial_volume.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
lh_white_surfarea <-
  unique(read_delim(str_c(path,"lh_white_surfarea.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
rh_white_surfarea <-
  unique(read_delim(str_c(path,"rh_white_surfarea.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
lh_lgi <-
  unique(read_delim(str_c(path,"lh_lgi.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
rh_lgi <-
  unique(read_delim(str_c(path,"rh_lgi.txt"),
    "\t",
    escape_double = FALSE,
    trim_ws = TRUE
  ))
...
data_h <- mutate(
  ...
  Session = str_sub(data_h$SUBJ_ID, 21, 21),
  SUBJ_ID = str_sub(data_h$SUBJ_ID, 9, 38))

data_h <- mutate(
  data_h,
  Longitudinal_correction = ifelse(str_sub(data_h$SUBJ_ID, 15, 18) == "long", "yes", "no"))

data_h$Session <- as.double(data_h$Session)
...
rm(lh_pial_surfarea, lh_pial_thickness, lh_pial_volume, lh_white_surfarea, lh_lgi, rh_pial_surfarea, rh_pial_thickness, rh_pial_volume, rh_white_surfarea, rh_lgi, gmvolume, localGI, thickness, totalarea, wmarea)
