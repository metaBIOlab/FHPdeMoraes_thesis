...
simulated_values <- as.data.frame(mvrnorm(n = 10000, mu = mean, Sigma = cov)) %>%
...
ggplot(filter(dados_datasetscomp_ALL_pred, ROI == "hemisphere", Family == "Hominidae"), aes(x = ExposedArea, y = log10(TotalArea*sqrt(AvgThickness)))) +
  geom_point(aes(color = Sample)) +
  scale_x_log10() +
