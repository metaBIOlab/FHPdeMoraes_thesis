...
# m.1 <- lme4::lmer(AvgThickness ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = dados_datasetscomp_lobes)
...
# m.1 <- lme4::lmer(localGI ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = dados_datasetscomp_lobes)
...
# m.1 <- lme4::lmer(K ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = dados_datasetscomp_lobes)
...
K_shiftc <- ggplot(filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL"), aes(x = Age, y = K, color = Sample, alpha = 0.4))+
  geom_point() +
  theme_pubr() + 
  guides(alpha = "none") + facet_grid(ROI ~. )
...
# m.1 <- lme4::lmer(S ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = dados_datasetscomp_lobes)
...
# m.1 <- lme4::lmer(I ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = dados_datasetscomp_lobes)
...
```{r}
amostra_Coef <- filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]") %>%
    group_by(ROI, Age_interval) %>%
    do(fit_amostra = tidy(lm(1/2 * log10(AvgThickness_shiftc) + log10(TotalArea_shiftc) ~ log10(ExposedArea_shiftc), data = ., na.action = na.omit), conf.int = TRUE, conf.level = 0.95)) %>% 
    unnest(fit_amostra)

amostras_Coef_age <- filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]") %>%
    group_by(Age_interval) %>%
    summarise(
        N = n_distinct(SUBJ),
        min_age = min(Age),
...
ggplot(filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]"), aes(y = K_shiftc, x = Age_interval), aes(y = K, x = Age_interval)) +
    geom_boxplot() + 
    theme_pubr() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)
