...
# full_join(aov_diag_I_age_decay_diag_TK_t) %>% mutate(signif = as.factor(ifelse(`p adj` < 0.05 | `p adj` == 0.05, "yes", "no")))
# 
# aov_summary_KSI_ADCTL <- as_tibble(aov_summary_KSI_ADCTL)
# 
...
#   theme_pubr()
# 
# ggplot(data = filter(aov_summary_KSI_ADCTL), aes(
#   x = ROI,
#   y = diff,
#   ymin = lwr,
#   ymax = upr,
#   color = Contrast,
#   linetype = as.factor(ifelse(signif == "no", 2, 1)))) +
#   geom_hline(yintercept = 0,
#              linetype = "11",
#              colour = "grey60") +
#   geom_errorbar(width = 0.1, position = position_dodge(width = 0.6)) +
...
#   facet_grid(morphological_parameter ~ Age_correction, labeller = label_both) +
#   labs(title = "95% family-wise confidence level", y =  "Differences in mean levels of Diagnostic", x = "ROI", caption = "only significant differences, p adj =< 0.05") +
#   theme_pubr() + theme(legend.title = element_blank()) + guides(linetype = FALSE)
# 
...
# plot_ly(
#   filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "70" | Age_interval10 == "80"
#   ),
#   x =  ~ mean.K,
#   z =  ~ mean.S,
#   y =  ~ mean.I,
...
# plot_ly(
#   filter(mean_K_I_S_ADCTL_zscore, Age_interval10 == "60" | Age_interval10 == "80"
#   ),
#   x =  ~ mean.K,
#   z =  ~ mean.S,
#   y =  ~ mean.I,
...
#   text = ~ Age_interval10
# ) %>% layout(scene = list(
#   xaxis = list(title = 'K (after age correction)'),
#   zaxis = list(title = 'S (after age correction)', scaleanchor = "x"),
#   yaxis = list(title = 'I (after age correction)', scaleanchor = "x")
# ))
# 
# 
# plot_ly(
...
# ) %>% layout(scene = list(
#   xaxis = list(title = 'K (after age correction)'),
#   yaxis = list(title = 'S (after age correction)', scaleanchor = "x")
# ))


# print(paste("Angle K-S: ",signif(angle(as.vector(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )$mean.K), as.vector(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )$mean.S)),2),"; Angle K-I: ",signif(angle(as.vector(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )$mean.K), as.vector(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )$mean.I)),2),"; Angle S-I: ", signif(angle(as.vector(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )$mean.S), as.vector(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )$mean.I)),2),"."))

# Vector_AD_CTL <- dplyr::select(filter(mean_K_I_S_ADCTL, Age_interval10 == "60" | Age_interval10 == "80"
#   ), c(Diagnostic, Age_interval10, mean.K, mean.S, mean.I)) %>% group_by(Diagnostic) %>% pivot_wider(names_from = Age_interval10, values_from = c(mean.K, mean.S, mean.I)) %>% mutate(K = mean.K_80-mean.K_60, S = mean.S_80 - mean.S_60, I = mean.I_80-mean.I_60)

# print(paste("Angle CTL-AD: ",signif(acos((filter(Vector_AD_CTL, Diagnostic == "CTL")$K%*%filter(Vector_AD_CTL, Diagnostic == "AD")$K+filter(Vector_AD_CTL, Diagnostic == "CTL")$S%*%filter(Vector_AD_CTL, Diagnostic == "AD")$S+filter(Vector_AD_CTL, Diagnostic == "CTL")$I%*%filter(Vector_AD_CTL, Diagnostic == "AD")$I)/(sqrt(filter(Vector_AD_CTL, Diagnostic == "CTL")$K^2+filter(Vector_AD_CTL, Diagnostic == "CTL")$S^2+filter(Vector_AD_CTL, Diagnostic == "CTL")$I^2)%*%sqrt(filter(Vector_AD_CTL, Diagnostic == "AD")$K^2+filter(Vector_AD_CTL, Diagnostic == "AD")$S^2+filter(Vector_AD_CTL, Diagnostic == "AD")$I^2))),2),"Angle CTL-MCI: ",signif(acos((filter(Vector_AD_CTL, Diagnostic == "CTL")$K%*%filter(Vector_AD_CTL, Diagnostic == "MCI")$K+filter(Vector_AD_CTL, Diagnostic == "CTL")$S%*%filter(Vector_AD_CTL, Diagnostic == "MCI")$S+filter(Vector_AD_CTL, Diagnostic == "CTL")$I%*%filter(Vector_AD_CTL, Diagnostic == "MCI")$I)/(sqrt(filter(Vector_AD_CTL, Diagnostic == "CTL")$K^2+filter(Vector_AD_CTL, Diagnostic == "CTL")$S^2+filter(Vector_AD_CTL, Diagnostic == "CTL")$I^2)%*%sqrt(filter(Vector_AD_CTL, Diagnostic == "MCI")$K^2+filter(Vector_AD_CTL, Diagnostic == "MCI")$S^2+filter(Vector_AD_CTL, Diagnostic == "MCI")$I^2))),2),"Angle MCI-AD: ",signif(acos((filter(Vector_AD_CTL, Diagnostic == "MCI")$K%*%filter(Vector_AD_CTL, Diagnostic == "AD")$K+filter(Vector_AD_CTL, Diagnostic == "MCI")$S%*%filter(Vector_AD_CTL, Diagnostic == "AD")$S+filter(Vector_AD_CTL, Diagnostic == "MCI")$I%*%filter(Vector_AD_CTL, Diagnostic == "AD")$I)/(sqrt(filter(Vector_AD_CTL, Diagnostic == "MCI")$K^2+filter(Vector_AD_CTL, Diagnostic == "MCI")$S^2+filter(Vector_AD_CTL, Diagnostic == "MCI")$I^2)%*%sqrt(filter(Vector_AD_CTL, Diagnostic == "AD")$K^2+filter(Vector_AD_CTL, Diagnostic == "AD")$S^2+filter(Vector_AD_CTL, Diagnostic == "AD")$I^2))),2)))


# ggplot(filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   ), aes(y = S, x = Diagnostic, color = Diagnostic, fill = Diagnostic)) + 
#   geom_boxplot() + stat_compare_means(method="anova") +
#     theme_pubr()
# 
# ggplot(filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   ), aes(y = S_age_decay, x = Diagnostic, color = Diagnostic, fill = Diagnostic)) + geom_boxplot() + stat_compare_means(method="anova") +
#     theme_pubr()
# 
#     ggplot(filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   ), aes(y = I, x = Diagnostic, color = Diagnostic, fill = Diagnostic)) + 
#   geom_boxplot() + stat_compare_means(method="anova") +
#     theme_pubr()
# 
# ggplot(filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   ), aes(y = I_age_decay, x = Diagnostic, color = Diagnostic, fill = Diagnostic)) + geom_boxplot() + stat_compare_means(method="anova") +
#     theme_pubr()


# aov_diag_K <- aov(K ~ Diagnostic*Age_interval10, data = (filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )))
# summary(aov_diag_K)
# aov_diag_K_diag_TK <- TukeyHSD(aov_diag_K)
# aov_diag_K_diag_TK_t <- mutate(as.data.frame(aov_diag_K_diag_TK$`Diagnostic:Age_interval10`), Contrast = c("MCI:60-AD:60","CTL:60-AD:60","AD:80-AD:60","MCI:80-AD:60","CTL:80-AD:60","CTL:60-MCI:60","AD:80-MCI:60","MCI:80-MCI:60","CTL:80-MCI:60","AD:80-CTL:60","MCI:80-CTL:60","CTL:80-CTL:60","MCI:80-AD:80","CTL:80-AD:80","CTL:80-MCI:80") , morphological_parameter = "K", ROI = "hemisphere", Age_correction = "no")
# 
# aov_diag_K_age_decay <- aov(K_age_decay ~ Diagnostic*Age_interval10, data = (filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )))
# summary(aov_diag_K_age_decay)
# aov_diag_K_age_decay_diag_TK <- TukeyHSD(aov_diag_K_age_decay)
# aov_diag_K_age_decay_diag_TK_t <- mutate(as.data.frame(aov_diag_K_age_decay_diag_TK$`Diagnostic:Age_interval10`), Contrast = c("MCI:60-AD:60","CTL:60-AD:60","AD:80-AD:60","MCI:80-AD:60","CTL:80-AD:60","CTL:60-MCI:60","AD:80-MCI:60","MCI:80-MCI:60","CTL:80-MCI:60","AD:80-CTL:60","MCI:80-CTL:60","CTL:80-CTL:60","MCI:80-AD:80","CTL:80-AD:80","CTL:80-MCI:80") , morphological_parameter = "K", ROI = "hemisphere", Age_correction = "yes")
# 
# aov_diag_S <- aov(S ~ Diagnostic*Age_interval10, data = (filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )))
# summary(aov_diag_S)
# aov_diag_S_diag_TK <- TukeyHSD(aov_diag_S)
# aov_diag_S_diag_TK_t <- mutate(as.data.frame(aov_diag_S_diag_TK$`Diagnostic:Age_interval10`), Contrast = c("MCI:60-AD:60","CTL:60-AD:60","AD:80-AD:60","MCI:80-AD:60","CTL:80-AD:60","CTL:60-MCI:60","AD:80-MCI:60","MCI:80-MCI:60","CTL:80-MCI:60","AD:80-CTL:60","MCI:80-CTL:60","CTL:80-CTL:60","MCI:80-AD:80","CTL:80-AD:80","CTL:80-MCI:80")  , morphological_parameter = "S", ROI = "hemisphere", Age_correction = "no")
# 
# aov_diag_S_age_decay <- aov(S_age_decay ~ Diagnostic*Age_interval10, data = (filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )))
# summary(aov_diag_S_age_decay)
# aov_diag_S_age_decay_diag_TK <- TukeyHSD(aov_diag_S_age_decay)
# aov_diag_S_age_decay_diag_TK_t <- mutate(as.data.frame(aov_diag_S_age_decay_diag_TK$`Diagnostic:Age_interval10`), Contrast = c("MCI:60-AD:60","CTL:60-AD:60","AD:80-AD:60","MCI:80-AD:60","CTL:80-AD:60","CTL:60-MCI:60","AD:80-MCI:60","MCI:80-MCI:60","CTL:80-MCI:60","AD:80-CTL:60","MCI:80-CTL:60","CTL:80-CTL:60","MCI:80-AD:80","CTL:80-AD:80","CTL:80-MCI:80")  , morphological_parameter = "S", ROI = "hemisphere", Age_correction = "yes")
# 
# aov_diag_I <- aov(I ~ Diagnostic*Age_interval10, data = (filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )))
# summary(aov_diag_I)
# aov_diag_I_diag_TK <- TukeyHSD(aov_diag_I)
# aov_diag_I_diag_TK_t <- mutate(as.data.frame(aov_diag_S_diag_TK$`Diagnostic:Age_interval10`), Contrast = c("MCI:60-AD:60","CTL:60-AD:60","AD:80-AD:60","MCI:80-AD:60","CTL:80-AD:60","CTL:60-MCI:60","AD:80-MCI:60","MCI:80-MCI:60","CTL:80-MCI:60","AD:80-CTL:60","MCI:80-CTL:60","CTL:80-CTL:60","MCI:80-AD:80","CTL:80-AD:80","CTL:80-MCI:80")  , morphological_parameter = "I", ROI = "hemisphere", Age_correction = "no")
# 
# aov_diag_I_age_decay <- aov(I_age_decay ~ Diagnostic*Age_interval10, data = (filter(dados_hemi_v1_DACTL, Age_interval10 == "60" | Age_interval10 == "80"
#   )))
# summary(aov_diag_I_age_decay)
# aov_diag_I_age_decay_diag_TK <- TukeyHSD(aov_diag_I_age_decay)
# aov_diag_I_age_decay_diag_TK_t <- mutate(as.data.frame(aov_diag_I_age_decay_diag_TK$`Diagnostic:Age_interval10`), Contrast = c("MCI:60-AD:60","CTL:60-AD:60","AD:80-AD:60","MCI:80-AD:60","CTL:80-AD:60","CTL:60-MCI:60","AD:80-MCI:60","MCI:80-MCI:60","CTL:80-MCI:60","AD:80-CTL:60","MCI:80-CTL:60","CTL:80-CTL:60","MCI:80-AD:80","CTL:80-AD:80","CTL:80-MCI:80"), morphological_parameter = "I", ROI = "hemisphere", Age_correction = "yes")
# 
# aov_summary_KSI_ADCTL <- full_join(aov_diag_K_diag_TK_t, aov_diag_K_age_decay_diag_TK_t) %>%
# full_join(aov_diag_S_diag_TK_t) %>%
# full_join(aov_diag_S_age_decay_diag_TK_t) %>%
...
# 
# ggplot(data = filter(aov_summary_KSI_ADCTL, `p adj` < 0.05 |
#                          `p adj` == 0.05, Contrast == "MCI:60-AD:60" | Contrast == "CTL:60-AD:60" | Contrast =="AD:80-AD:60" | Contrast =="CTL:60-MCI:60" | Contrast =="MCI:80-MCI:60" | Contrast =="MCI:80-AD:80" | Contrast =="CTL:80-AD:80" | Contrast =="CTL:80-MCI:80"| Contrast == "CTL:80-CTL:60"), aes(
#   x = ROI,
#   y = diff,
#   ymin = lwr,
#   ymax = upr,
...
#   geom_errorbar(width = 0.1, position = position_dodge(width = 0.6)) +
#   geom_point(position = position_dodge(width = 0.6)) +
#   scale_x_discrete(limits=c("hemisphere")) + 
#   coord_flip() +
#   facet_grid(morphological_parameter~ Age_correction, labeller = labeller(Age_correction = hospital_names)) +
#   labs(title = "95% family-wise confidence level", y =  "Differences in mean levels of Diagnostic and Age decade", x = "ROI")+
#   theme_pubr()
# 
# 
# aov_summary_KSI_ADCTL %>% filter(`p adj` < 0.05 |
#                          `p adj` == 0.05, Contrast == "MCI:60-AD:60" | Contrast == "CTL:60-AD:60" | Contrast =="AD:80-AD:60" | Contrast =="CTL:60-MCI:60" | Contrast =="MCI:80-MCI:60" | Contrast =="MCI:80-AD:80" | Contrast =="CTL:80-AD:80" | Contrast =="CTL:80-MCI:80" | Contrast == "CTL:80-CTL:60") %>%
#   kable(digits =3) %>%
#   kable_styling()
#     
```
...
multinomaL <- multinom(Age ~ K, data = train.data)
m.multi.nova <-
  multinom(Age ~ K, data = train.data)
  stargazer(m.multi.nova, type = "text")

  z <-
    summary(m.multi.nova)$coefficients / summary(m.multi.nova)$standard.errors
...
t(coef.multi)
#Previsoes
predicted.classes.multi.nova <- m.multi.nova %>% predict(test.data, type = "class")

#Model accuracy
mean(predicted.classes.multi.nova == test.data$Age)

# Summary
confusionMatrix(predicted.classes.multi.nova, test.data$Age)

#ROC
multiclass.roc(
  as.numeric(test.data$Age),
  as.numeric(predicted.classes.multi.nova),
  percent = F,
  ci.alpha = 0.9,
  stratified = FALSE,
  plot = TRUE,
