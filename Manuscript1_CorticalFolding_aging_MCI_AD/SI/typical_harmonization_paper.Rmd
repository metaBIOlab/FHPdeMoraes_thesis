...
N_SUBJ <- dados_datasetscomp %>% group_by(Diagnostic) %>% summarise(N_SUBJ = n_distinct(SUBJ))
```

## AvgThickness \~ Age

```{r}
m.1 <- lme4::lmer(AvgThickness ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(TotalArea ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(ExposedArea ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(localGI ~ Age * Diagnostic + (1|Sample:Diagnostic), data = dados_datasetscomp)
...
m.1 <- lme4::lmer(K ~ Age * Diagnostic + (1|Sample:Diagnostic), data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(S ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
# dados_datasetscomp <- full_join(dados_datasetscomp, re, by=c("Sample")) %>%  mutate(S_shiftc = S - S_shift)
```

## I \~ Age

```{r}
m.1 <- lme4::lmer(I ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
# ggsave("fig_slope_ageinterval.pdf", plot = fig_slope_ageinterval, dpi=1200, width = 18, height = 18, units = "cm", device = "pdf")
```
...
```{r}
m.1 <- lme4::lmer(AvgThickness_shiftc ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(localGI_shiftc ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(K_shiftc ~ Age * Diagnostic + (1|Sample:Diagnostic), data = dados_datasetscomp)
...
```{r}
K_shiftc <- ggplot(filter(dados_datasetscomp, Diagnostic == "CTL"), aes(x = Age, y = K, color = Sample, alpha = 0.4))+
  geom_point() +
  theme_pubr() + 
  guides(alpha = "none") + 
  theme(
...
K_shiftd <- ggplot(filter(dados_datasetscomp, Diagnostic == "CTL"), aes(x = Age, y = K_shiftc, color = Sample, alpha = 0.4))+
  geom_point() +
    guides(alpha = "none") + 
  theme_pubr() + 
  theme(
...
K_shift <- ggarrange(K_shiftc, K_shiftd ,labels = c("A","B"), nrow = 2, ncol = 1, font.label = list(size = 11), common.legend = TRUE, legend = "top")
K_shift
...
```{r}
m.1 <- lme4::lmer(S_shiftc ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r}
m.1 <- lme4::lmer(I_shiftc ~ Age * Diagnostic + (1|Sample:Diagnostic) , data = dados_datasetscomp)
...
```{r echo=FALSE}
dados_datasetscomp_lobes <- filter(dados_datasetscomp, ROI != "hemisphere", Diagnostic == "CTL" | Diagnostic == "MCI"| Diagnostic == "AD", Sample != "IDOR-CCD-Control")
...
N_SUBJ <- dados_datasetscomp_lobes %>% group_by(Diagnostic, ROI) %>% summarise(N_SUBJ = n_distinct(SUBJ))
```

### AvgThickness \~ Age

```{r}
ggplot(dados_datasetscomp_lobes, aes(x = Age, y = AvgThickness, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.title = element_text(size = 11),
...
```{r}
ggplot(dados_datasetscomp_lobes, aes(x = Age, y = K, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.title = element_text(size = 11),
    axis.text = element_text(size = 10), text = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
...
```{r echo=FALSE}
m.1 <- lme4::lmer(AvgThickness ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(AvgThickness_shiftc = AvgThickness - T_shift)
```

### TotalArea \~ Age

```{r echo=FALSE}
m.1 <- lme4::lmer(TotalArea ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(TotalArea_shiftc = TotalArea - AT_shift)
```

### ExposedArea \~ Age

```{r echo=FALSE}
m.1 <- lme4::lmer(ExposedArea ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(ExposedArea_shiftc = ExposedArea - AE_shift)
```
...
```{r echo=FALSE}
m.1 <- lme4::lmer(localGI ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(GI_shiftc = localGI - GI_shift)
```

### K \~ Age
```{r echo=FALSE}
m.1 <- lme4::lmer(K ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(K_shiftc = K - K_shift)
```

```{r}
K_shiftc <- ggplot(filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL"), aes(x = Age, y = K, color = Sample, alpha = 0.4))+
  geom_point() +
  theme_pubr() + 
  guides(alpha = "none") + 
  theme(
...
K_shiftd <- ggplot(filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL"), aes(x = Age, y = K_shiftc, color = Sample, alpha = 0.4))+
  geom_point() +
    guides(alpha = "none") + 
  theme_pubr() + 
  theme(
...
# ggsave("K_shift_lobes.pdf", plot = K_shift_lobes, width = 18, height = 22, units = "cm", device = "pdf")
```

### S \~ Age

```{r echo=FALSE}
m.1 <- lme4::lmer(S ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(S_shiftc = S - S_shift)
```

### I \~ Age

```{r echo=FALSE}
m.1 <- lme4::lmer(I ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# dados_datasetscomp_lobes <- full_join(dados_datasetscomp_lobes, re) %>% mutate(I_shiftc = I - I_shift)
```
...
## After harmonization
```{r}
ggplot(dados_datasetscomp_lobes, aes(x = Age, y = AvgThickness_shiftc, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.title = element_text(size = 11),
    axis.text = element_text(size = 10), text = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
...
```{r}
ggplot(dados_datasetscomp_lobes, aes(x = Age, y = K_shiftc, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.title = element_text(size = 11),
    axis.text = element_text(size = 10), text = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
...
```{r}
amostra_Coef <- filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "00-05", Age_interval != "95-100") %>%
    group_by(ROI, Age_interval) %>%
    do(fit_amostra = tidy(lm(1/2 * log10(AvgThickness_shiftc) + log10(TotalArea_shiftc) ~ log10(ExposedArea_shiftc), data = ., na.action = na.omit), conf.int = TRUE, conf.level = 0.95)) %>% 
    unnest(fit_amostra)

amostras_Coef_age <- filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "00-05", Age_interval != "95-100") %>%
    group_by(Age_interval) %>%
    summarise(
        N = n_distinct(SUBJ),
        min_age = min(Age),
...
ggplot(filter(dados_datasetscomp_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "00-05", Age_interval != "95-100"), aes(y = K_shiftc, x = Age_interval), aes(y = K, x = Age_interval)) +
    geom_boxplot() + 
    theme_pubr() + 
    theme(axis.title = element_text(size = 11),
          axis.text = element_text(size = 10),
...
# ggsave("fig_slope_lobes_ageinterval.pdf", plot = fig_slope_lobes_ageinterval, dpi=1200, width = 18, height = 18, units = "cm", device = "pdf")
```

### AvgThickness \~ Age
```{r echo=FALSE}
m.1 <- lme4::lmer(AvgThickness_shiftc ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic)  + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
### GI \~ Age
```{r echo=FALSE}
m.1 <- lme4::lmer(localGI_shiftc ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic)  + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
### K \~ Age
```{r echo=FALSE}
m.1 <- lme4::lmer(K_shiftc ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
```{r echo=FALSE}
m.1 <- lme4::lmer(S_shiftc ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
```{r echo=FALSE}
m.1 <- lme4::lmer(I_shiftc ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = dados_datasetscomp_lobes)
...
# ggsave("fig_sampleeff_c.pdf", plot = fig_sampleeff, dpi=1200, width = 18, height = 18, units = "cm", device = "pdf")
...
# ggsave("fit_K_lobes_c.pdf", plot = fit_K_lobes, dpi=1200, width = 11, height = 15, units = "cm", device = "pdf")

# ggsave("fit_S_lobes_c.pdf", plot = fit_S_lobes, dpi=1200, width = 11, height = 15, units = "cm", device = "pdf")

# ggsave("fit_I_lobes_c.pdf", plot = fit_I_lobes, dpi=1200, width = 11, height = 15, units = "cm", device = "pdf")
```
