...
```{r comp longitudinal correction, echo=FALSE, message=FALSE, warning=FALSE}
...
Aqui comparo metodos de extracao das medidas morfologicas, geradas pelo script acima que chamei de com correao longitudinal (esss sujeitos sao acompanhados no tempo e voltam a cada ~ 2 anos). - Nao vamos incluir no primeiro paper por isso nao inclui no relatorio
...
#### SLOPE? - comparacao com outras amostras ja publicadas
...
###### SLOPE? - comapracao com outras amostras ja publicas - corrigido pela idade
...
todas as variaveis com "age_decay" sao corrigidas pela idade
...
Aqui usei um t-test para diferenciar os diagnosticos. Qual o teste mais correto?
...
### Podemos localizar partes com maior detrimento por idade e doenca? - estudo de outras ROIs menores, lobos: frontal, temporal, parietal e ocipital
...
A medida da girificacao nos lobos precisa de uma correcao matematica por um problema de topologia.
...
### normalizado
Aqui gostaria de mostrar se existe correlacao entre os testes clinicos e a girificacao, provavelmente focando em algum lobo específico já pré determinado. 
Tanto os testes clinicos quanto a girificacao sao afetados pela idade. Como eu incluo esse fator em uma correlacao?
...
Aqui comecei a tentar os testes de normalidade e os modelos.

## verificacao da normalidade ----
Fiz o teste de nromalidade dos residuos, era isso mesmo?
...
```{r glm, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
dados_v1$Diagnostic <- relevel(dados_v1$Diagnostic, "CONTROLE")
logit <- glm(Diagnostic ~ logAvgThickness + K + Age + ROI, data = dados_v1)
