...
dados_datasetscomp <- full_join(dados_datasetscomp, re) %>% mutate(AvgThickness_shiftc = AvgThickness - T_shift)
...
dados_datasetscomp <- full_join(dados_datasetscomp, re) %>% mutate(TotalArea_shiftc = TotalArea - AT_shift)
...
dados_datasetscomp <- full_join(dados_datasetscomp, re) %>% mutate(ExposedArea_shiftc = ExposedArea - AE_shift)
